...
class AngryFoe(foe.Foe):
    ...
    def __init__(self, pos, game_map, spriteImg="angryFoe.png"):
        super(AngryFoe, self).__init__(pos, game_map, spriteImg)
    ...
    def move(self, direction, game_map):
        if AngryFoe.POTENTIALFIELD:
            ...
            minimum_potential = AngryFoe.POTENTIALFIELD[self.pos[1]][self.pos[0]]
            for i in current_cell.available_direction:
                ...
                if AngryFoe.POTENTIALFIELD[cell_location[1]][cell_location[0]] < minimum_potential:
                    minimum_potential = AngryFoe.POTENTIALFIELD[cell_location[1]][cell_location[0]]
    ...
    def recalculate(cls, cells):
        ...
        print AngryFoe.POTENTIALFIELD

    @classmethod
    def recalculate_rec(cls, cells, pos):
        newPotential = AngryFoe.POTENTIALFIELD[pos[1]][pos[0]] + 1
        current_cell = cells[pos[1]][pos[0]]
        for i in current_cell.available_direction:
            movement = AngryFoe.SWITCH[i]
            cell_location = ((pos[0] + movement[0]) % (len(cells)), (pos[1] + movement[1]) % (len(cells[0])))
            if (AngryFoe.POTENTIALFIELD[cell_location[1]][cell_location[0]] == 0 or
                    AngryFoe.POTENTIALFIELD[cell_location[1]][cell_location[0]] > newPotential):
                AngryFoe.POTENTIALFIELD[cell_location[1]][cell_location[0]] = newPotential
